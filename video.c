...
static int queue_picture(player_stat_t *is, AVFrame *src_frame, double pts,
                         double duration, int64_t pos) {
  frame_t *vp;
  if (!(vp = frame_queue_peek_writable(&is->video_frm_queue))) return -1;
...
static int video_decode_frame(AVCodecContext *p_codec_ctx,
      ...
      if (ret < 0) {
        if (ret == AVERROR_EOF) {
          av_log(NULL, AV_LOG_INFO,
                 "video avcodec_receive_frame(): the decoder has been fully "
                 "flushed\n");
          avcodec_flush_buffers(p_codec_ctx);
          return 0;
        } else if (ret == AVERROR(EAGAIN)) {
          // av_log(NULL, AV_LOG_INFO, "video avcodec_receive_frame(): output is
          // not available in this state - user must try to send new input\n");
          break;
        } else {
          av_log(NULL, AV_LOG_ERROR,
                 "video avcodec_receive_frame(): other errors\n");
          continue;
        }
      } else {
...
static int video_decode_thread(void *arg) {
  ...
  AVRational frame_rate =
      av_guess_frame_rate(is->p_fmt_ctx, is->p_video_stream, NULL);
  ...
  while (1) {
    got_picture =
        video_decode_frame(is->p_vcodec_ctx, &is->video_pkt_queue, p_frame);
    if (got_picture < 0) {
      goto exit;
    }
    ...
    duration = (frame_rate.num && frame_rate.den
                    ...
                    : 0);  // 当前帧播放时长
    pts = (p_frame->pts == AV_NOPTS_VALUE)
              ? NAN
              : p_frame->pts * av_q2d(tb);  // 当前帧显示时间戳
    ret = queue_picture(is, p_frame, pts, duration,
                        p_frame->pkt_pos);  // 将当前帧压入frame_queue
...
static double compute_target_delay(double delay, player_stat_t *is) {
  ...
  // 视频时钟与同步时钟(如音频时钟)的差异，时钟值是上一帧pts值(实为：上一帧pts +
  // 上一帧至今流逝的时间差)
  ...
  if (!isnan(diff)) {
    if (diff <= -sync_threshold)  // 视频时钟落后于同步时钟，且超过同步域值
      delay = FFMAX(
          0,
          delay +
              diff);  // 当前帧播放时刻落后于同步时钟(delay+diff<0)则delay=0(视频追赶，立即播放)，否则delay=delay+diff
    else if (
        diff >= sync_threshold &&
        delay >
            AV_SYNC_FRAMEDUP_THRESHOLD)  // 视频时钟超前于同步时钟，且超过同步域值，但上一帧播放时长超长
      delay =
          delay +
          diff;  // 仅仅校正为delay=delay+diff，主要是AV_SYNC_FRAMEDUP_THRESHOLD参数的作用
...
static void update_video_pts(player_stat_t *is, double pts, int64_t pos,
                             int serial) {
...
static void video_display(player_stat_t *is) {
  ...
  // slice: 图像中一片连续的行，必须是连续的，【顺序由顶部到底部】或【由底部到顶部】
  // stride/pitch:
  ...
  is->sdl_video.rect.y =
      (is->sdl_video.window_height - is->sdl_video.height) / 2;
...
retry:
  ...
  /* dequeue the picture */
  lastvp =
      frame_queue_peek_last(&is->video_frm_queue);  // 上一帧：上次已显示的帧
  ...
  last_duration =
      vp_duration(is, lastvp, vp);  // 上一帧播放时长：vp->pts - lastvp->pts
  delay = compute_target_delay(
      last_duration, is);  // 根据视频时钟和同步时钟的差值，计算delay值
  ...
  if (!isnan(vp->pts)) {
    ...
    update_video_pts(is, vp->pts, vp->pos,
                     vp->serial);  // 更新视频时钟：时间戳、时钟时间
  }
  SDL_UnlockMutex(is->video_frm_queue.mutex);

  // 是否要丢弃未能及时播放的视频帧
  if (frame_queue_nb_remaining(&is->video_frm_queue) >
      1)  // 队列中未显示帧数>1(只有一帧则不考虑丢帧)
  {
    frame_t *nextvp =
        frame_queue_peek_next(&is->video_frm_queue);  // 下一帧：下一待显示的帧
    duration = vp_duration(is, vp,
                           nextvp);  // 当前帧vp播放时长 = nextvp->pts - vp->pts
    // 当前帧vp未能及时播放，即下一帧播放时刻(is->frame_timer+duration)小于当前系统时刻(time)
    if (time > is->frame_timer + duration) {
      frame_queue_next(
          &is->video_frm_queue);  // 删除上一帧已显示帧，即删除lastvp，读指针加1(从lastvp更新到vp)
      goto retry;
    }
  }
...
static int open_video_playing(void *arg) {
  ...
  buf_size = av_image_get_buffer_size(
      AV_PIX_FMT_YUV420P, is->p_vcodec_ctx->width, is->p_vcodec_ctx->height, 1);
  ...
  is->sdl_video.window =
      SDL_CreateWindow("simple ffplayer",
  ...
  is->sdl_video.texture = SDL_CreateTexture(
      is->sdl_video.renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING,
      is->sdl_video.rect.w, is->sdl_video.rect.h);
...
static int open_video_stream(player_stat_t *is) {
  ...
  is->sdl_video.height_width_ratio =
      (double)(1.0 * is->sdl_video.height) / is->sdl_video.width;
