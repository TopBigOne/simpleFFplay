...
static int demux_init(player_stat_t *is) {
  ...
  // 1.2
  // 搜索流信息：读取一段视频文件数据，尝试解码，将取到的流信息填入p_fmt_ctx->streams
...
static int demux_thread(void *arg) {
  ...
  while (1) {
    ...
    if (is->seek_req) {
      ...
      ret = avformat_seek_file(p_fmt_ctx, -1, seek_min, seek_target, seek_max,
    ...
    if (is->audio_pkt_queue.size + is->video_pkt_queue.size > MAX_QUEUE_SIZE ||
        (stream_has_enough_packets(is->p_audio_stream, is->audio_idx,
                                   &is->audio_pkt_queue) &&
         stream_has_enough_packets(is->p_video_stream, is->video_idx,
                                   &is->video_pkt_queue))) {
      /* wait 10 ms */
    ...
    if (ret < 0) {
      ...
      {
        // 输入文件已读完，则往packet队列中发送NULL
        // packet，以冲洗(flush)解码器，否则解码器中缓存的帧取不出来
        if (is->video_idx >= 0) {
          packet_queue_put_nullpacket(&is->video_pkt_queue, is->video_idx);
        }
        if (is->audio_idx >= 0) {
